...
<head>
  ...
  <title><?= tr('meta.lb.title') ?></title>
  <meta name="description" content="<?= tr('meta.lb.desc') ?>" data-i18n-meta="meta.lb.desc"/>
  ...
  <?php require_once $_SERVER['DOCUMENT_ROOT'].'/includes/seo.php'; sk_seo_head(['title'=>tr('meta.lb.title'),'desc'=>tr('meta.lb.desc')]); ?>
</head>
<body>
...
<header class="livre-blanc-hero" style="background:var(--dark);padding:100px clamp(20px,5vw,80px);">
  <div class="sk-wrap" style="display:grid;grid-template-columns:1.2fr 0.8fr;gap:64px;align-items:center;">
    <div>
      ...
      <p style="font-size:15px;color:rgba(255,255,255,.6);line-height:1.75;margin-bottom:28px;" data-i18n="lb.hero.sub"><?= tr('lb.hero.sub') ?></p>
      ...
      <a href="#telechargement" class="btn-primary" data-i18n="lb.dl.btn">📥 <?= tr('lb.dl.btn') ?></a>
    </div>
    <div style="display:flex;justify-content:center;">
      <div style="position:relative;">
        <div style="width:200px;height:280px;background:linear-gradient(135deg,#1a1a2e 0%,#2d2d48 100%);border-radius:8px 16px 16px 8px;box-shadow:6px 6px 0 rgba(232,86,58,.4),12px 12px 0 rgba(232,86,58,.15),0 32px 80px rgba(0,0,0,.5);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;padding:28px;border-left:5px solid var(--orange);">
          ...
          <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--orange);text-align:center;line-height:1.4;" data-i18n="lb.cover.label"><?= tr('lb.cover.label') ?><br><span data-i18n="lb.cover.line2"><?= tr('lb.cover.line2') ?></span></div>
          <div style="font-size:11px;color:rgba(255,255,255,.5);text-align:center;"><?= tr('lb.pages') ?> · <?= tr('lb.free') ?></div>
...
<section id="telechargement" style="padding:80px clamp(20px,5vw,80px);">
  <div class="sk-wrap" style="display:grid;grid-template-columns:1fr 1fr;gap:64px;align-items:start;">
    <div>
      <span class="section-label" data-i18n="lb.dl.section"><?= tr('lb.dl.section') ?></span>
      ...
      async function handleLbSubmit(e) {
        ...
        if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
          errEl.textContent = <?= json_encode(tr('lb.js.invalid_email')) ?>;
    ...
    <div>
      <span class="section-label" data-i18n="lb.chapters.h2"><?= tr('lb.chapters.h2') ?></span>
      <h3 style="font-family:var(--font-display);font-size:1.3rem;font-weight:700;color:var(--dark);margin-bottom:24px;" data-i18n="lb.chapters.sub"><?= tr('lb.chapters.sub') ?></h3>
